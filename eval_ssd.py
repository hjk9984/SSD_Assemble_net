...
from vision.ssd.assemble_vgg import create_assemble_vgg_ssd, create_assemble_vgg_ssd_predictor
...
if __name__ == '__main__':
    ...
    if args.dataset_type == "voc":
        dataset = VOCDataset(args.dataset, keep_difficult=True)
    ...
    elif args.net == "asb-vgg-ssd":
        net = create_assemble_vgg_ssd(len(class_names), is_test=True)
    else:
    ...
    if args.net == 'vgg16-ssd':
        ...
        predictor = create_assemble_vgg_ssd_predictor(net, nms_method=args.nms_method, device=DEVICE)
    else:
        logging.fatal("The net type is wrong. It should be one of vgg16-ssd, mb1-ssd and mb1-ssd-lite.")
        parser.print_help(sys.stderr)
        sys.exit(1)
    ...
    for i in range(len(dataset)):
        ...
        print(boxes.size(), labels.size(), probs.size())
